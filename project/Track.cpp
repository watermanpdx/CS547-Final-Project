...
{
    //Initialize contained bird object
    bird.Initialize();
...
Track::Draw(void)
{
    ...
    // Draw the track (controled by parameter)
    if (TRACK_VISIBLE) { glCallList(track_list); }
...
//Set contained bird object flap phase
void
Track::SetBirdPhase(float offset)
{
    bird.SetPhaseOffset(offset);
}

coord3D*
Track::GetBirdPositionPointer(void)
{
    return &bird_position;
}

coord3D*
Track::GetBirdDirectionPointer(void)
{
    return &bird_direction;
}

//Update object animation parameters
void
Track::Update(float dt)
{
    ...
    //Store positional info for get() methods
    bird_position.x = point[0];
    bird_position.y = point[1];
    bird_position.z = point[2];
    bird_direction.x = deriv[0];
    bird_direction.y = deriv[1];
    bird_direction.z = deriv[2];
}